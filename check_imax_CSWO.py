...
def check_shows() -> tuple:
    # ── Method 1: All-venues API, filter by CSWO + IMAX ──────────────────────
    print(f"📡 Method 1: All-venues API (filtering for {CINEMA_CODE} + IMAX)...")
    try:
        ...
        if cswo_block:
            ...
            # Filter for IMAX specifically
            is_imax = any("imax" in s.lower() or "imax" in f.lower()
                         for s in screens for f in formats)
            if times and is_imax:
                print(f"   ✅ Method 1: IMAX shows found at {CINEMA_NAME}!")
                return True, times
            elif times:
                print(f"   ⚠️  Shows found but NOT IMAX (probably 4DX/2D) — not alerting")
            else:
    ...
    # ── Method 2: Movie page, look for CSWO + IMAX in JSON ───────────────────
    ...
    try:
        ...
        if cswo_in_page:
            ...
            has_imax = "imax" in block.lower()
            has_4dx  = "4dx" in block.lower()
            print(f"   {CINEMA_CODE} in page: True | IMAX: {has_imax} | 4DX: {has_4dx} | Times: {times[:5]}")
            if times and has_imax:
                print(f"   ✅ Method 2: IMAX shows confirmed!")
                return True, times
            elif times and not has_imax:
                print(f"   ⚠️  Shows exist but no IMAX (4DX={has_4dx}) — not alerting")
        else:
...
def main():
    ...
    if found:
        ...
        print("   (4DX may be available but we only alert for IMAX)")
